...
# @FileName: smart_selector_demo_v3.py
...
if __name__ == "__main__":
    ...
    # requirement = "我想提取固收+近1年年化收益超过0.02，且近1年最大回撤不超过50%的产品，开始时间为'2024-01-01'"
    ...
    while user_input:
        ...
        if ("筛选条件修改" in intent_list) and ("新增指标" not in intent_list): intent_list.insert(
            intent_list.index("筛选条件修改"), "新增指标")
        ...
        for user_intent in intent_list:
            if user_intent == "量化知识咨询":
                ...
                rsp = indicator_modification_agent.run(input=user_input, indicator_list=indicator_list)
                rsp = rsp.to_dict()
                ...
                for i, iIndicator in enumerate(adjusted_indicator_list):
                    if (iIndicator != indicator_dict[iIndicator["指标代码"]]) and (not iIndicator["在指标库"]):
                        iAlgorithm = gen_indicator_calculator_rslt(iIndicator, user_info="",
                                                                   agent=indicator_calculator_sa_agent)
                ...
                rsp = indicator_adding_agent.run(input=user_input, indicator_list=indicator_list,
                ...
                print(f"新增后的指标列表: {pformat(indicator_list)}")
                ...
                for iInd, iIndicator in indicator_algorithm.items():
                    ...
                    for iQuery in indicator_list:
                        if iQuery["指标名称"] == iIndicator["指标名称"]:
                            ...
                            break
                    rsp = indicator_algorithm_modification_agent.run(input=user_input,
                                                                     indicator_name=iIndicator["指标名称"],
                                                                     indicator_args=iIndicatorArgs)
                    rsp = rsp.to_dict()
                ...
                while task_list:
                    ...
                    if iTask == "指标提取":
                        print(f"\n======================== 系分生成: {iTask} ========================")
                        rsp = indicator_query_sa_agent.run(requirement=user_input,
                        ...
                        print(f"指标列表: {pformat(indicator_list)}")
                    elif iTask == "标的筛选":
                        if not all((iQuery.get("在指标库", False) or (iQuery["指标代码"] in indicator_algorithm)) for
                                   iQuery in indicator_list):
                        ...
                        if all((iQuery.get("在指标库", False) or (iQuery["指标代码"] in indicator_algorithm)) for iQuery
                               in indicator_list):
                        ...
                        for i, iQuery in enumerate(indicator_list):
                            ...
                            iAlgorithm = gen_indicator_calculator_rslt(iQuery, user_info="",
                                                                       agent=indicator_calculator_sa_agent)
    ...
    sleep(2)
    task_info = {
        "待计算指标": [ind["指标名称"] for ind in indicator_list if not ind["在指标库"]],
