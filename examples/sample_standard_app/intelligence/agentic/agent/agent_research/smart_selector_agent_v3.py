...
# @Time    : 2025/1/12 00:01
# @Author  : tonymhl
# @email   : mahongli.mhl@antgroup.com
# @Version : Python3
# @File    : smart_selector_agent_v3.py
# @Software: PyCharm
...
# @FileName: smart_selector_agent_v3.py
...
# LOGGER = Logger(__name__, LoggerType.SLS)
...
def execute_task_with_verification(task_type, code_str, data, task, kwargs):
    ...
    if not debug_result.to_dict()['output']:
        LOGGER.error(f"代码验证失败，需重新进行校验")
        return None
    else:
        debug_info = debug_result.to_dict()['output']
        LOGGER.info(f"{debug_info}")
        bool_value_str = extract_debug_result(debug_info)
        ...
        if bool_value is False:
            ...
            if retry == 'y':
                # 更新 kwargs 中的 error_info
                kwargs['error_info'] = debug_info
                if task_type == "filter":
...
class IndicatorExtractionTask:
    def __init__(self, agent, max_retries=3):
        self.agent = agent
        self.max_retries = max_retries

    def execute(self, indicator_list):
        LOGGER.info("开始指标提取")
        indicator_data = []
        retry_count = 0
        for i, iQuery in enumerate(indicator_list):
            ...
            while retry_count < self.max_retries:
                try:
                    LOGGER.info(f"开始提取指标 {i + 1}/{len(indicator_list)}: {iQuery['指标名称']}")
                    res = self.agent.run(input=iQuery, error_info=error_info)
                    ...
                    retry_count += 1
                    LOGGER.error(f"提取失败 (尝试 {retry_count}/{self.max_retries}): {str(e)}")
                    if retry_count < self.max_retries:
                        ...
                        if res is not None:
                            LOGGER.info(f"使用错误信息优化后提取成功")
                            indicator_data.append(res.to_dict()["data"])
                            continue
                    ...
                    if retry_count == self.max_retries:
                        ...
                        retry_count = 0
        LOGGER.info("指标提取完成")
        return {"output": indicator_data, "status": "success" if indicator_data else "partial_failure"}


class IndicatorFilteringTask:
    def __init__(self, agent, max_retries=3):
        self.agent = agent
        self.max_retries = max_retries

    def execute(self, user_input, indicator_list, indicator_data, sec_filter, error_info):
        LOGGER.info("开始指标筛选任务")
        code_str = None
        filtered_data = None
        retry_count = 0

        while filtered_data is None and retry_count < self.max_retries:
            try:
                if code_str is None:
                ...
                if filtered_data is not None:
                    ...
                    return {"output": filtered_data, "run_code": code_str, "status": "success"}
            except Exception as e:
                retry_count += 1
                LOGGER.error(f"筛选失败 (尝试 {retry_count}/{self.max_retries}): {str(e)}")
                if retry_count < self.max_retries:
                    print("\n尝试使用错误信息优化筛选代码...")
                    ...
                    if res is not None:
                        LOGGER.info(f"使用错误信息优化后筛选成功")
                        output = res.to_dict()['output']
                        LOGGER.info(f"filter_indicator_agent产出代码: \n {output}")
                        error_info = output
                        # 提取代码
                        code_str = extract_code_from_output(output)
        ...
        return {"output": filtered_data,
                "run_code": code_str,
                "status": "partial_failure" if filtered_data is None else "success"}
...
class IndicatorCalculationTask:
    ...
    def execute(self, user_input, indicator_list, indicator_algorithm, error_info):
        LOGGER.info("开始指标计算任务")
        code_str = None
        calculation_results = {}
        for i, iQuery in enumerate(indicator_list):
            if iQuery["库编码"] is not None:
                continue
            ...
            while calculation_result is None and retry_count < self.max_retries:
                try:
                    ...
                    if calculation_result is not None:
                        ...
                        LOGGER.info(f"计算结果: \n {calculation_result}")
                        calculation_results[iQuery["指标代码"]] = calculation_result
                except Exception as e:
                    retry_count += 1
                    LOGGER.error(f"计算失败 (尝试 {retry_count}/{self.max_retries}): {str(e)}")
                    if retry_count < self.max_retries:
                        print(f"\n尝试使用错误信息优化计算代码...")
                        res = handle_code_generation_error(
                            e,
                    ...
                    if retry_count == self.max_retries:
                        ...
                        retry_count = 0
        LOGGER.info("指标计算完成")
        return {"output": calculation_results,
                "run_code": code_str,
                "status": "success" if calculation_results else "partial_failure"}
...
class SmartSelectorAgent(Agent):
    ...
    def execute(self, input_object: InputObject, agent_input: dict):
        ...
        if (user_input.lower() == "yes") and self._Memeory["indicator_list"] and self._Memeory["all_filled"]:
            print(append_list(OutputList, "系分生成完毕, 下面生成执行代码"))

            # =================================================
            # 初始化相关任务类
            indicator_extraction_agent = AgentManager().get_instance_obj('indicator_nscode_extraction_agent')
            indicator_filtering_agent = AgentManager().get_instance_obj('filter_indicator_agent')
            indicator_calculation_agent = AgentManager().get_instance_obj('indicator_calculation_agent')

            max_retries = 3

            extraction_task = IndicatorExtractionTask(indicator_extraction_agent, max_retries=max_retries)
            filtering_task = IndicatorFilteringTask(indicator_filtering_agent, max_retries=max_retries)
            calculation_task = IndicatorCalculationTask(indicator_calculation_agent, max_retries=max_retries)
            ...
            if len(indicator_list_new) > 0:
                ...
                extraction_result = extraction_task.execute(indicator_list_new)
                if extraction_result["status"] == "success":
                    indicator_data = extraction_result["output"]
                else:
                    print("部分指标提取失败或无数据")
            ...
            if has_non_empty_data:
                ...
                filtering_result = filtering_task.execute(
                    user_input,
                    indicator_list,
                    indicator_data,
                    sec_filter,
                ...
                if filtering_result["status"] == "success":
                    filtered_data = filtering_result["output"]
                else:
                    print("指标筛选部分失败或无数据")
                print("指标筛选完成")
            else:
            ...
            calculation_result = calculation_task.execute(
                user_input,
                indicator_list,
                indicator_algorithm,
                error_info=''
            ...
            if calculation_result["status"] == "success":
                calculation_outputs = calculation_result["output"]
            else:
                print("指标计算部分失败或无数据")
            print("指标计算完成")

            # 将各部分结果添加到OutputList
            OutputList.append(f"指标提取结果: {indicator_data}")
            OutputList.append(f"指标筛选结果: {filtered_data}")
            OutputList.append(f"指标计算结果: {calculation_outputs}")

            Output = {
                "output": "\n".join(OutputList),
                "output_list": OutputList,
                "status": self._Memeory,
                "ext_info": ExtInfo
            }
            print(Output)
            return Output
        ...
        for user_intent in intent_list:
            if user_intent == "量化知识咨询":
                ...
                print(append_list(OutputList, f"调整后的衍生指标算法: {pformat(output.get('adjusted_indicator_algorithm', {}))}"))
...
if __name__ == '__main__':
    ...
    agent = AgentManager().get_instance_obj('smart_selector_agent_v3')

    # requirement = "我想提取私募基金近1年年化超额超过0.8，且近3个月正收益概率超过90%的产品，开始时间为'2025-01-01'"
    requirement = "我想提取公募近1年年化收益超过0.02，且近1年最大回撤不超过50%的产品，开始时间为'2024-01-01'"
